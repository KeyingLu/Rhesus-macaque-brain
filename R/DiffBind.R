...
library(magrittr)
p <- lapply(names(sharePeaksAnno), function(x){
  stat <- sharePeaksAnno[[x]]@annoStat
  stat <- data.frame(stat, reproducible=x)
}) %>% Reduce(rbind, .)

write.table(p, "SourceData/Fig.5a.txt",
...
rowpeaks <- peaks[CA1_enrHtmp@row_order]
CA1_mat <- data.frame(CA1_enrHtmp@matrix)
rownames(CA1_mat) <- rowpeaks
colnames(CA1_mat) <- paste0("CA1_", colnames(CA1_mat))

PCG_mat <- data.frame(PCG_enrHtmp@matrix)
rownames(PCG_mat) <- rowpeaks
colnames(PCG_mat) <- paste0("PCG_", colnames(PCG_mat))

other_mat <- data.frame(other_enrHtmp@matrix)
rownames(other_mat) <- rowpeaks
colnames(other_mat) <- paste0("other_", colnames(other_mat))

p <- cbind(CA1_mat, PCG_mat, other_mat)
p <- data.frame(peaks=rowpeaks, p)

write.table(p, "SourceData/Fig.5b.txt",
            col.names = T, row.names = F, quote=F, sep="\t")
...
library(magrittr)
p <- lapply(names(peakAnno_list), function(x){
  stat <- peakAnno_list[[x]]@annoStat
  stat <- data.frame(stat, reproducible=x)
}) %>% Reduce(rbind, .)

write.table(p, "SourceData/Fig.S7e.txt",
            col.names = T, row.names = F, quote=F, sep="\t")
...
setwd("/mnt/data2/Rhesus_brain/")
p <- df
colnames(p) <- c("CA1-open", "PCG-open")
write.table(p, "SourceData/Fig.S7f.txt",
            col.names = T, row.names = F, quote=F, sep="\t")
