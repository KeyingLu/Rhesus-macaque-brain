...
Rhesus_Gene_norCounts_save <- data.frame(GeneID=rownames(Rhesus_Gene_norCounts), 
                            Rhesus_Gene_norCounts)
write.table(Rhesus_Gene_norCounts_save, "SourceData/SourceData_Fig.3.txt",
...
write.table(p, "SourceData/Fig.S4a.txt",
...
ggplot(pp, aes(x=SR, y=expression, fill=Module, color=Module)) +
# ggplot(pp, aes(x=SR, y=expression, fill=Module)) +
  geom_boxplot(outlier.size = 0.7) +
  geom_point(position = position_jitterdodge(dodge.width = 0.75, jitter.width=1), 
  ...
  stat_summary(geom = "point", shape=23, fill="white",size=1,
               fun.y="median") +
  guides(fill=F, color=F) +
  xlab("") +
  ylab("Module eigengene expression")

ggsave("revise0530/gene_count_res/Module_eigengene_expression_exclude_MB_1226.pdf", 
       width = 9.6, height = 5)


p <- pp
colnames(p) <- c("Region", "Module", "Expression")
write.table(p, "SourceData/Fig.3a.S4b-c.txt",
            col.names = T, row.names = F, quote=F, sep="\t")
...
p <- pp
colnames(p)[4] <- "region"
# write.table(p, "SourceData/Fig.S4d.txt",
#             col.names = T, row.names = F, quote=F, sep="\t")
write.table(p, "SourceData/Fig.S4e.txt",
            col.names = T, row.names = F, quote=F, sep="\t")
...
p <- data.frame(gene=rownames(dfheat), dfheat)
write.table(p, "SourceData/Fig.3c.txt",
            col.names = T, row.names = F, quote=F, sep="\t")
