...
textMatrix = paste(signif(modTraitCor, 2), "\n(", signif(modTraitP, 1), ")", sep = "")
dim(textMatrix) = dim(modTraitCor)
save(modTraitCor, modTraitP, file="revise0530/gene_count_res/WGCNA_modTraitCor_for_ID_cortex.RData")
...
pheatmap(
  ...
  filename="revise0530/gene_count_res/Net_modules_and_ID_cortex_traits_cor_pheatmap.pdf",
  col=colorRampPalette(rev(brewer.pal(11,"RdBu")))(40),
  breaks = seq(-1,1,length=40),
  legend_breaks = c(-1, -0.5, 0, 0.5, 1),
  # cellwidth = 9,
...
ggplot(p, aes(x=ID, y=mean, fill=Module)) +
  ...
  scale_fill_manual(values = c(wes_palette("Royal2")[c(1,3,5)], 
                               wes_palette("GrandBudapest1"))) +
  facet_wrap(~Module,ncol=2, scales="free_y") +
  geom_errorbar((aes(ymin=mean-se, ymax=mean+se))) +
  guides(fill=F) +
  xlab("") +
  ylab("Module eigengene expression")

ggsave("revise0530/gene_count_res/Module_eigengene_expression_ID_cortex.pdf", 
       width = 6, height = 5)
